...
title(ax_geometry, 'Site Geometry And Editable Trajectories')
...
title(ax_altitude, 'Altitude Profile')
...
for idx = 1 : numel(bistatic_tracks)
    ...
    plot(ax_range, relative_time_s, double(bistatic_tracks(idx).R_excess_m(:)) ./ 1e3, ...
        'Color', target_colors(idx, :), ...
        'LineWidth', 1.8, ...
        'DisplayName', localBistaticLabel(bistatic_tracks(idx)))
end
grid(ax_range, 'on')
xlabel(ax_range, 'Relative Time [s]')
ylabel(ax_range, 'Bistatic Range Excess [km]')
title(ax_range, 'Bistatic Truth Preview')
...
title(ax_doppler, 'Doppler Truth Preview')
...
target_label = char(string(target_struct.target_id) + " (" + string(target_struct.callsign) + ")");
end

function target_label = localBistaticLabel(track_struct)
target_label = char(string(track_struct.hex) + " (" + string(track_struct.callsign) + ")");
end
