%% RF Budget Analysis - Passive Radar System (Newton - CBS Tower)
% Signals of Opportunity Aircraft Tracking using HDTV (ATSC) Transmissions
%
% SYSTEM CONFIGURATION:
%   - Transmitter: CBS Tower Newton, Channel 35 (596-602 MHz, center: 599 MHz)
%   - Location: N 42° 18' 37", W 71° 14' 12" (NAD83)
%   - ERP: 922 kW horizontal (59.6 dBW), 184 kW vertical (52.6 dBW)
%   - Tower Height: 1274' AGL / 1417' AMSL
...
%   1. CBS Tower is VERY POWERFUL (922 kW ERP)
...
%   5. Bistatic link budget needs calculation for new transmitter location
%
% Updated: December 8, 2025
% Author: RF Budget Analysis Tool
% Reference: RFBudgetUpdateProgress_Newton.md for detailed analysis
%
%==========================================================================
...
InputFreq = 599e6;              % Channel 35 CBS Tower center frequency (Hz)
...
% CBS Tower Newton transmitter parameters
Tx_ERP_Horizontal_kW = 922;     % Horizontal ERP (kW)
Tx_ERP_Vertical_kW = 184;       % Vertical ERP (kW)
Tx_ERP_Horizontal_dBW = 10*log10(Tx_ERP_Horizontal_kW * 1000);  % 59.6 dBW
Tx_TowerHeight_ft = 1274;       % AGL
Tx_Latitude = 42.310278;        % degrees N
Tx_Longitude = -71.236667;      % degrees W

% Target signal power from bistatic link budget analysis
% ⚠️  TODO: Calculate based on CBS Tower parameters and bistatic geometry
...
fprintf('   Needs bistatic link budget calculation for CBS Tower.\n\n');
...
BP_Filter_Loss_dB = 1.5;        % ZABP-587-S+ insertion loss @ 599 MHz (estimated higher)
...
Include_HP_Filter = true;       % Set to false if not using high-pass filter

fprintf('⚠️  FILTER OFFSET WARNING:\n');
fprintf('   ZABP-587-S+ centered at %d MHz, target is %.0f MHz\n', ...
    BP_Filter_Center_MHz, InputFreq/1e6);
fprintf('   Offset: %.0f MHz - insertion loss may be %.1f dB (vs 1.0 dB nominal)\n', ...
    BP_Filter_Offset_MHz, BP_Filter_Loss_dB);
fprintf('   Consider measuring actual insertion loss or using ZABP-603-S+\n\n');
...
fprintf('CBS Tower Newton - Channel 35 @ 599 MHz\n');
fprintf('========================================\n\n');

% Display CBS Tower transmitter information
fprintf('--- CBS TOWER TRANSMITTER ---\n');
fprintf('Location: N %.6f°, W %.6f°\n', Tx_Latitude, abs(Tx_Longitude));
fprintf('Tower Height: %d ft AGL / %d ft AMSL\n', Tx_TowerHeight_ft, ...
    Tx_TowerHeight_ft + 143);  % 143 ft ground elevation approx
...
if Include_HP_Filter
    ...
    Power_AfterHP = Power_AfterMismatch;
end
...
% Overall title
sgtitle(sprintf('RF Budget Analysis - CBS Tower Newton Ch 35 (599 MHz)\nSystem NF: %.2f dB | Total Gain: %.2f dB | SNR: %.2f dB', ...
...
fprintf('   - Insertion loss estimated at %.1f dB (vs 1.0 dB nominal)\n', BP_Filter_Loss_dB);
fprintf('   - RECOMMENDATION: Measure actual insertion loss with VNA\n');
fprintf('   - Alternative: ZABP-603-S+ (centered at 603 MHz, only 4 MHz offset)\n\n');

% CBS Tower high power warning
fprintf('⚠️  CRITICAL: Very High Transmitter Power\n');
fprintf('   - CBS Tower ERP: %.0f kW (%.1f dBW horizontal)\n', ...
    Tx_ERP_Horizontal_kW, Tx_ERP_Horizontal_dBW);
...
fprintf('     * CBS Tower location: %.6f°N, %.6f°W\n', Tx_Latitude, abs(Tx_Longitude));
...
fprintf('For detailed analysis, see: RFBudgetUpdateProgress_Newton.md\n');
fprintf('========================================\n\n');