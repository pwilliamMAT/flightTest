...
%[text] This notebook is the review entry point for understanding how the Interacting Multiple Model (IMM) estimator was configured, what was allowed to change during tuning, and how the frozen tuned estimator performed. Run All loads the saved full result by default, so reviewing the result does not repeat optimization.
...
stage4ETuningPath = fullfile(stage4EOutputFolder, "stage4EFilterTuning.mat");
%%
%[text] ## 1. Load Or Recompute The Benchmark
%[text] Set |regenerateStage4EResults = true| to recompute benchmark scores with the existing frozen tuning. Set |retuneStage4EFilters = true| only when intentionally replacing the validation-only tuning artifact; retuning is much slower and changes the evidence used by the later test evaluation.
...
stage4ETuningCandidates = [ ...
    string(stage4EResults.paths.tuning); ...
    stage4ETuningPath; ...
    fullfile( ...
        stage4EProjectRoot, ...
        "artifacts", ...
...
stage4ETuningCandidateIndex = find( ...
    isfile(stage4ETuningCandidates), ...
    1, ...
    "first");
if isempty(stage4ETuningCandidateIndex)
    error( ...
        "Stage4E:MissingTuningArtifact", ...
        "No frozen Stage 4E tuning artifact was found.");
end
stage4ETuningPath = ...
    stage4ETuningCandidates(stage4ETuningCandidateIndex);
%%
%[text] ## 2. How The Project Reached Stage 4E
%[text] The project began with one-step ADS-B motion prediction. Native |constvel| established a strong physics baseline, while Stage 3A supplied the first frozen neural predictor. Stage 4B-Post then expanded and audited the ADS-B dataset, and Stage 4C trained scratch and warm-start neural candidates on an aircraft-disjoint split; neither neural candidate displaced the native constant-velocity baseline.
%[text] Stage 4D froze the warm network and characterized one-step behavior under known synthetic motion, held-out ADS-B intervals, and missing reports. That work deliberately did not answer whether prediction errors would accumulate inside a recursive estimator. Stage 4E closes that gap by placing the native CV, CA, and CT models, their native IMM combination, and the frozen warm transition inside complete filters with matched initialization, timestamps, corrections, and scoring rows.
%[text] This progression matters: the IMM was not selected because of a one-step plot, and the neural network was not retrained during Stage 4E. The final comparison is a controlled recursive-filter benchmark built on frozen inputs and validation-only tuning.
%%
%[text] ## 3. What Recursive Filtering Adds
...
%%
%[text] ## 4. Five Estimators, One Input Contract
...
%%
%[text] ## 5. IMM Configuration Before Tuning
%[text] |helperInitializeStage4EFilter| creates three native |trackingEKF| filters using |constvel|, |constacc|, and |constturn|, then combines them with |trackingIMM| and the native |switchimm| state-conversion function. All models begin with equal probability. The scalar transition setting of 0.95 becomes 95% probability of remaining in the same model and 2.5% probability of changing to either alternative.
%[text] The tables below are generated from the initializer itself. Process-noise diagonals are shown in each native model's own noise coordinates; the turn model therefore includes a turn-rate-noise entry that the other models do not have.
stage4EConfigurationAttributes = struct( ...
    InitialVelocity=zeros(3, 1), ...
    InitialCovarianceDiag=ones(6, 1));
stage4EConfigurationDetection = objectDetection( ...
    0, ...
    zeros(3, 1), ...
    MeasurementNoise=eye(3), ...
    ObjectAttributes=stage4EConfigurationAttributes);
stage4EInitialIMM = helperInitializeStage4EFilter( ...
    stage4EConfigurationDetection, ...
    "native_imm");
stage4EInitialModelNames = string(stage4EInitialIMM.ModelNames(:));
stage4EInitialProcessNoiseDiagonals = cellfun( ...
    @(filter) diag(filter.ProcessNoise).', ...
    stage4EInitialIMM.TrackingFilters, ...
    UniformOutput=false);
stage4EInitialIMMConfiguration = table( ...
    stage4EInitialModelNames, ...
    repmat("trackingEKF", numel(stage4EInitialModelNames), 1), ...
    stage4EInitialProcessNoiseDiagonals, ...
    stage4EInitialIMM.ModelProbabilities(:), ...
    VariableNames=[ ...
        "motionModel", ...
        "filterClass", ...
        "processNoiseDiagonal", ...
        "initialModelProbability"]);
stage4EInitialTransitionProbabilities = ...
    stage4EInitialIMM.TransitionProbabilities;
disp(stage4EInitialIMMConfiguration)
disp(stage4EInitialTransitionProbabilities)
%%
%[text] ## 6. Validation-Only Tuning And Its Result
%[text] |trackingFilterTuner| uses |fmincon| with the SQP algorithm. It tunes only the diagonal process-noise entries for every estimator and the IMM transition probabilities. Initial state covariance, IMM model probabilities, UKF sigma-point parameters, measurement noise, motion equations, and neural-network weights remain fixed.
%[text] Three validation events contribute at most 120 state pairs each. The custom cost is the root mean square of normalized error in the common physical state |\[x, vx, y, vy, z, vz\]|, using scales |\[100 m, 10 m/s, 100 m, 10 m/s, 150 m, 5 m/s\]|. Lower validation cost is better. No test row participates in tuning.
loadedStage4ETuning = load(stage4ETuningPath, "stage4ETuning");
stage4ETuning = loadedStage4ETuning.stage4ETuning;
stage4ETuningSummary = stage4EResults.tuningSummary;
stage4ETuningCosts = sortrows( ...
    stage4ETuningSummary.costTable, ...
    "validationCost");
stage4ETunedIMM = stage4ETuning.parameters.native_imm;
stage4ETunedProcessNoiseDiagonals = cellfun( ...
    @(filter) diag(filter.ProcessNoise).', ...
    stage4ETunedIMM.TrackingFilters, ...
    UniformOutput=false);
stage4EIMMProcessNoiseComparison = table( ...
    stage4EInitialModelNames, ...
    stage4EInitialProcessNoiseDiagonals, ...
    stage4ETunedProcessNoiseDiagonals, ...
    VariableNames=[ ...
        "motionModel", ...
        "initialProcessNoiseDiagonal", ...
        "tunedProcessNoiseDiagonal"]);
stage4EIMMTransitionComparison = array2table( ...
    [ ...
        stage4EInitialTransitionProbabilities, ...
        stage4ETunedIMM.TransitionProbabilities], ...
    RowNames=cellstr(stage4EInitialModelNames), ...
    VariableNames=[ ...
        "initialToCV", ...
        "initialToCA", ...
        "initialToCT", ...
...
disp(stage4EIMMProcessNoiseComparison)
disp(stage4EIMMTransitionComparison)
disp(stage4ETuningCosts)
%%
%[text] ## 7. Synthetic Truth Separates Motion And Outage Effects
...
%%
%[text] ## 8. Held-Out ADS-B Is A Scoring Proxy
...
%%
%[text] ## 9. IMM Headline Results
%[text] The tuned native IMM has the lowest aggregate posterior position RMSE on degraded canonical truth and on the held-out ADS-B baseline proxy. The table also shows how its ADS-B proxy error changes as reports are removed. The burst-outage result is much worse than random dropout because one long interval forces extended open-loop coasting.
stage4EIMMHeadlineResults = stage4EMetrics( ...
    stage4EMetrics.modelID == "native_imm" & ...
    stage4EMetrics.estimateStage == "posterior" & ...
    ( ...
        ( ...
            stage4EMetrics.domain == "synthetic_canonical" & ...
        ...
        ( ...
            stage4EMetrics.domain == "real_adsb" & ...
            ismember( ...
                stage4EMetrics.profile, ...
                [ ...
                    "baseline", ...
                    "random_dropout_10", ...
                    "random_dropout_25", ...
                    "burst_outage"]))), ...
    ["domain", "profile", "positionRMSEMeters"]);
disp(stage4EIMMHeadlineResults)
%%
%[text] ## 10. Motion, Dropout, Coasting, And Runtime
...
%%
%[text] ## 11. Recursive Integrity And Statistical Consistency
...
%%
%[text] ## 12. Representative Globe Paths
...
%%
%[text] ## 13. Conclusion And Boundary
