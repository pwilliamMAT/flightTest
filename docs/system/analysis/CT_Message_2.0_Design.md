...
**Date:** 2026-09-26. **Status:** design proposal, for approval before the ICD is edited and code changes.
...
| d | Removed from each opportunity: `summary` (use `windows[]`), `history` (always null on the wire), `emitter_enabled` and `observer_can_receive` (always true, since CT sends only those), `opportunity_id` (= `observer_id:emitter_id`), `observer_name` and `transmitter_site_id` (configuration lookups) | Derivable, constant, or never sent |
...
|   | – every property is present in every message; | |
