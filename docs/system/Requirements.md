...
> **Status: DRAFT baseline for owner review (proposed by CR-10, 2026-09-26; CR-10 stays open). The baseline as a whole is not accepted. Individual owner decisions of 2026-09-26 are recorded in §7.**
...
- No number is invented. Where a value is not known it is **TBD**. The one value set from a measurement (DR-TIME-1, 0.1 s) cites the measurement and was accepted by the owner on 2026-09-26.
...
| MN-3 | Give every dataset independent truth good enough to judge MATLAB function outputs (detections, tracks) against it. | MG-2 | System | I, A | Partial | ADS-B is packaged with collections; timing tolerance DR-TIME-1 (0.1 s, accepted); truth separation CR-9 accepted, labels not yet implemented |
...
| MN-5 | Keep every claim that a function or the system works traceable to evidence, with its evidence class and claim boundary, and never overstated. | MG-1, MG-2 | Process | I | Partial | Reporting rules ([howToUpdateReports.md](../../reporting/howToUpdateReports.md)); cued-evidence labelling accepted (CR-9), not yet implemented |
...
| SR-05 | Every host that timestamps IQ, ADS-B truth or cues shall keep UTC within the timing tolerance of DR-TIME-1. | Truth is only useful if its timestamps line up with the IQ. | MN-3 | Time Source, RC, AR, CT | T, A | Partial | NTP and the 0.1 s tolerance accepted by the owner (2026-09-26); one check at +0.05 to +0.09 s ([Verification_Log.md](Verification_Log.md) 2026-09-25 21:07); GPS/PPS not locked (DR-TIME-3) |
...
| SR-13 | Every reported claim shall state its evidence class and claim boundary, and evidence from cued or cue-aided processing shall be labelled as such (DR-TS). | Prevents modeled, diagnostic or cue-aided evidence from being read as independent or operational evidence. | MN-5 | Process | I | Partial | Reporting family rules; truth-separation rule accepted (CR-9, 2026-09-26), labels not yet implemented |
...
| DR-DATA-6 | Each package shall record `collection_basis` and, for cued collections, the `cue_ref` (CR-9, rule TS-1). | Cued data must never be mistaken for independent detection evidence. | SR-10, SR-13 | RC, RM | I | Proposed | CR-9 (accepted); ICD §3.1, §3.5 still Proposed |
| DR-DATA-7 | Each package shall record the capture bandwidth and the list of emitters (RF channels) the capture covers. | A capture may be widened (for example to 12 MHz) to take in adjacent channels, and a site may host several emitters. | SR-10, SR-13 | RM, RC | I | Proposed | CR-9 follow-up: ICD §3.1 `collection_task` and §3.5 `capture_record` need an emitter list and a capture bandwidth |
...
| DR-DEP-4 | The exceptions to MATLAB implementation shall be named: CT (Python), system services (chrony, gpsd, dump1090) and ESP32 firmware. | Keeps MG-1 honest about what is not MATLAB. | SR-12, MN-1 | CT, AR, Time Source, AC | I | Verified | Named in [System_Architecture.md](System_Architecture.md); the owner accepted the Python CT as a named exception under MG-1 on 2026-09-26 |

### 5.6 Truth separation (parents SR-07, SR-13; rule accepted in CR-9, 2026-09-26)

Terms: a **site** (tower) is a transmitter site; it may host several **emitters**, each one DTV transmitter on one RF channel (`emitter_id`). A cue names an emitter.

| ID | Requirement | Rationale | Parent | Allocation | Method | Status | Evidence |
|---|---|---|---|---|---|---|---|
| DR-TS-1 | Every capture shall be labelled with its `collection_basis`: `cued`, `uncued`, `calibration` or `survey`. The label applies to the whole capture, including every channel in it; a cued capture carries the `cue_ref` naming the cued emitter. | Cues choose when, and with which emitter, to collect; a capture may be widened (for example to 12 MHz) to take in adjacent channels. | SR-13, SR-10 | RM, RC | I | Proposed | CR-9 rule TS-1 (accepted); the carrying messages, ICD §3.1 and §3.5, are still Proposed |
| DR-TS-2 | Every detection and track product shall be labelled with its `truth_use`: `truth_blind` or `cue_aided`, judged per product. | The Tracker may use cues to help association. | SR-13 | SP, TR | I | Proposed | CR-9 rule TS-2 (accepted); ICD §3.8 and §3.9 are still Proposed |
| DR-TS-3 | Only `truth_blind` products shall count as independent detection or tracking evidence. | Evidence must not contain the truth it is scored against. | SR-07, SR-13 | SP, TR, process | I | Not started | CR-9 rule TS-3 (accepted); no cued collection or product exists yet |
| DR-TS-4 | Detection probability from cued collections shall be reported as conditional on the cue, including for products on emitters other than the cued one. False-alarm rates shall state the collections and cells they were measured on. | Cued collections are not a random sample of the sky. | SR-13 | Process | I, A | Not started | CR-9 rule TS-4 (accepted) |
| DR-TS-5 | Cue-aided association and tracking shall be scored separately from truth-blind results, and never reported as independent tracking performance. | Cues and ADS-B truth come from the same receiver. | SR-13 | TR, RD, process | I | Not started | CR-9 rule TS-5 (accepted) |
| DR-TS-6 | Every detection and track shall record the emitter (`emitter_id`, RF channel) it came from. | A wide or multi-emitter capture holds products from several emitters; each must be traceable to its illuminator. | SR-07, SR-13 | SP, TR | I | Proposed | CR-9 rule TS-2 (accepted); `emitter_id` is already in ICD §3.8 and §3.9 (Proposed) |
...
## 7. Owner decisions and open points

Decided 2026-09-26: internet NTP with the 0.1 s tolerance (DR-TIME-1); the Python CT is an accepted named exception (DR-DEP-4); CR-9 accepted with the emitter and wide-capture amendment (DR-TS, DR-DATA-7); the Report 02 acceptance tests (A-B-A reference test, 30 s drop test) stay in Report 02 and do not become derived requirements.

Open:

1. Accept, change or reject each requirement; the baseline stays DRAFT while CR-10 is open.
2. Values still **TBD**: pointing tolerance (SR-04), ADS-B lead and tail (DR-DATA-4), content and approver of the data-release statement (SR-11, DR-DATA-5).
