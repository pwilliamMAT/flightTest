...
| CR-9 | Truth separation for cued collections and cue-aided association | Proposal | Architecture (new design rule); ICD §3.1, §3.5, §3.8, §3.9 (labels); Requirements DR-TS | Open (need accepted 2026-09-26; rule text for review) |
| CR-10 | Adopt a requirements baseline | Proposal | New controlled document [Requirements.md](Requirements.md) | Open (DRAFT for review) |
...
- **Why:** the reporting family's rule is that ADS-B truth is post hoc and must not steer map formation, thresholding, non-maximum suppression or detection. Two things now change that picture, as the owner confirmed on 2026-09-26:
  - CT cues choose **when** to collect and **with which DTV tower**, and may set antenna pointing.
...
- **Need accepted (owner, 2026-09-26).** The rule text below is proposed for review.
- **Definitions:**
  - *Collection basis:* how a capture was chosen.
    - `cued`: time, illuminator or pointing came from a CT cue.
...
- **Proposed rule:**
  1. **TS-1, label collections.** Every capture carries `collection_basis`. A cued capture also carries the `cue_ref` of the prediction that caused it (track, prediction revision, observer, emitter, window start). Where: ICD §3.1 `collection_task`, §3.5 `capture_record`, and the dataset manifest.
  2. **TS-2, label products.** Every detection list and track report carries `truth_use`. Where: ICD §3.8 `detection_list` and §3.9 `track_report`.
  ...
  4. **TS-4, rates.**
     ...
     - False-alarm rates state the collections and cells they were measured on.
  ...
  7. **TS-7, reports.** Every reported result based on cued data states its `collection_basis` and `truth_use`. `cue_aided` evidence is never classed as operational evidence.
- **Changes proposed on acceptance:**
  - a "Truth separation" design rule in System_Architecture.md;
  - the labels in ICD §3.1, §3.5, §3.8 and §3.9 (all still Proposed messages, so no released schema changes);
  - Requirements DR-TS-1 to DR-TS-5 and DR-DATA-6 become accepted.
- **Decide:** accept the rule text, or amend it.
...
- **Proposal:** [Requirements.md](Requirements.md) is a DRAFT baseline:
  ...
  - 28 derived requirements.

  Each requirement has a rationale, parent, allocation, verification method, status, and evidence. No values are invented; unknown values are TBD, and the one proposed value (DR-TIME-1, 0.1 s) cites its measurement.
