...
if verbose
    fprintf('  Detecting DPI lag (first %d-CPI sub-chunk [%d x %d])...\n', ...
...
max_detector_blocks = max(1, ceil(N_chunks / max(config.max_nci_looks, 1)));
detector_blocks = repmat(struct( ...
    'block_num',       {}, ...
    'look_count',      {}, ...
    't_part_center_s', {}, ...
    'rdm_whitened_db', {}, ...
    'row_nf_db',       {}, ...
    'abs_nf_db',       {}), 1, max_detector_blocks);
...
for k_chunk = 1 : N_chunks
    ...
    if block_is_full || is_last_chunk
        ...
        detector_blocks(block_num) = struct( ...
            'block_num',       block_num, ...
...
detector_blocks = detector_blocks(1:block_num);
