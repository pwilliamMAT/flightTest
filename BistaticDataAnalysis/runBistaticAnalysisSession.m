...
%    'Use2DGeographicFallback'       Use stable geoaxes maps instead of
%                                    WebGL-backed geoglobe figures during
%                                    replay. Default: true
...
addParameter(p, 'Use2DGeographicFallback', true, @(x) islogical(x) && isscalar(x));
...
addParameter(p, 'PlotLegacyTruthComparison', true, @islogical);
addParameter(p, 'PlotDetectionTruthFigure', true, @islogical);
...
analysisSetup.use_2d_geographic_fallback = logical(opts.Use2DGeographicFallback);
...
if analysisSetup.use_2d_geographic_fallback
    fprintf('  geo plots    : geoaxes 2-D fallback\n');
else
    fprintf('  geo plots    : geoglobe 3-D requested\n');
end
...
analysis_output = struct( ...
    ...
    'use_2d_geographic_fallback', logical(analysisSetup.use_2d_geographic_fallback));
...
wrapper_opts = struct( ...
    ...
    'use_2d_geographic_fallback', logical(opts.Use2DGeographicFallback), ...
    ...
    'plot_track_comparison', logical(opts.PlotLegacyTruthComparison), ...
    'plot_detection_truth_figure', logical(opts.PlotDetectionTruthFigure));
end
...
wrapper_opts = localBuildWrapperOptions(struct( ...
    ...
    'PlotLegacyTruthComparison', true, ...
    'PlotDetectionTruthFigure', true));
