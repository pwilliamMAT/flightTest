...
%                                    Default: 'core'
%    'PlotAssessmentFigures'         Override the profile default.
%    'PlotPerPartRDM'                Override the profile default.
%    'PlotEllipseGlobe'              Override the profile default.
%    'PlotTrackerGlobe'              Override the profile default.
%    'PlotTrackLegend'               Override the profile default.
%    'PlotInteractiveRDViewer'       Override the profile default.
%    'PlotTruthDiagnostics'          Override the profile default.
...
analysisSetup.visualization_options = helperResolveVisualizationProfile( ...
    'VisualizationProfile', opts.VisualizationProfile, ...
    'PlotAssessmentFigures', opts.PlotAssessmentFigures, ...
    'PlotPerPartRDM', opts.PlotPerPartRDM, ...
    'PlotEllipseGlobe', opts.PlotEllipseGlobe, ...
    'PlotTrackerGlobe', opts.PlotTrackerGlobe, ...
    'PlotTrackLegend', opts.PlotTrackLegend, ...
    'PlotInteractiveRDViewer', opts.PlotInteractiveRDViewer, ...
    'PlotTruthDiagnostics', opts.PlotTruthDiagnostics, ...
    'Force2DGeographicFallback', opts.Force2DGeographicFallback, ...
    'ExpectedPartCount', numel(analysisSetup.data_parts));
...
fprintf('  viz profile  : %s\n', char(analysisSetup.visualization_options.profile));
fprintf('  max figures  : %d\n', analysisSetup.visualization_options.max_expected_open_figure_count);
...
analysis_output = struct( ...
    ...
    'figure_inventory', analysisSetup.visualization_options.figure_inventory, ...
    'ranked_crash_causes', analysisSetup.visualization_options.ranked_crash_causes);
...
if exist('visualization', 'var') && isstruct(visualization)
    analysis_output.visualization = visualization;
    if isfield(visualization, 'figure_inventory')
        analysis_output.figure_inventory = visualization.figure_inventory;
    end
    if isfield(visualization, 'ranked_crash_causes')
        analysis_output.ranked_crash_causes = visualization.ranked_crash_causes;
    end
end
...
analysis_output.restart_commands = helperBuildSessionRestartCommands( ...
    analysisSetup.session_id, ...
    analysis_output.truth_diag_snapshot, ...
    analysis_output.detector_replay_snapshot, ...
    'VisualizationProfile', analysis_output.visualization.profile);
localPrintRestartCommands(analysis_output.restart_commands);
...
tf = strlength(mode) == 0 || any(strcmp(mode, ["full", "core", "headless"]));
end
...
wrapper_opts = struct( ...
    ...
    'visualization_profile', string(opts.VisualizationProfile), ...
    'plot_assessment_figures', opts.PlotAssessmentFigures, ...
    'plot_per_part_rdm', opts.PlotPerPartRDM, ...
    'plot_ellipse_globe', opts.PlotEllipseGlobe, ...
    'plot_tracker_globe', opts.PlotTrackerGlobe, ...
    'plot_track_legend', opts.PlotTrackLegend, ...
    'plot_interactive_rdm_viewer', opts.PlotInteractiveRDViewer, ...
    'plot_truth_diagnostics', opts.PlotTruthDiagnostics, ...
    'force_2d_geographic_fallback', opts.Force2DGeographicFallback);
end
...
wrapper_opts = localBuildWrapperOptions(struct( ...
    ...
    'PlotAssessmentFigures', [], ...
    'PlotPerPartRDM', [], ...
    'PlotEllipseGlobe', [], ...
    'PlotTrackerGlobe', [], ...
    'PlotTrackLegend', [], ...
    'PlotInteractiveRDViewer', [], ...
    'PlotTruthDiagnostics', [], ...
    'Force2DGeographicFallback', []));
...
function localPrintRestartCommands(restart_commands)
if ~isstruct(restart_commands)
    return
end

fprintf('\nFast restart commands (run from %s):\n', char(restart_commands.working_folder));
if isfield(restart_commands, 'truth_only') && strlength(restart_commands.truth_only) > 0
    fprintf('  truth only   : %s\n', char(restart_commands.truth_only));
end
if isfield(restart_commands, 'detector_only') && strlength(restart_commands.detector_only) > 0
    fprintf('  detector only: %s\n', char(restart_commands.detector_only));
end
if isfield(restart_commands, 'full_session') && strlength(restart_commands.full_session) > 0
    fprintf('  full session : %s\n', char(restart_commands.full_session));
end
fprintf('\n');
end
