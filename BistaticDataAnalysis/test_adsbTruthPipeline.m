...
diag_source_bb = fullfile(tmp_dir, 'synthetic_diag_source.bb');
localWriteSyntheticBasebandFile(diag_source_bb, fs, fc);
data_parts_diag = repmat({diag_source_bb}, 1, N_parts_sim);

range_axis_diag = linspace(0, 120e3, 220).';
doppler_axis_diag = linspace(-400, 400, 201);
config_diag = struct( ...
    'fc', fc, ...
    'fs', fs, ...
    'N_slow_cpi', 200, ...
...
part_results_diag = repmat(struct( ...
    ...
    'doppler_axis', doppler_axis_diag, ...
    'detector_blocks', struct([])), 1, N_parts_sim);

all_track_dets_diag = zeros(0, 6);
det_counter = 0;

for ip = 1 : N_parts_sim
    rdm_after_diag = -8 + 0.3 * randn(numel(range_axis_diag), numel(doppler_axis_diag));
    dets_ip = zeros(0, 5);
    block_times_ip = t_abs_query( ...
        t_abs_query >= part_start_offsets_diag(ip) & ...
        t_abs_query < part_end_offsets_diag(ip));
    blocks_ip = repmat(struct( ...
        'block_num', 0, ...
        'look_count', 1, ...
        't_part_center_s', 0, ...
        'rdm_whitened_db', zeros(numel(range_axis_diag), numel(doppler_axis_diag)), ...
        'row_nf_db', zeros(numel(range_axis_diag), 1), ...
        'abs_nf_db', 0), 1, numel(block_times_ip));

    for ib = 1 : numel(block_times_ip)
        t_block = block_times_ip(ib);
        rdm_white_blk = 0.2 * randn(numel(range_axis_diag), numel(doppler_axis_diag));
        occupied_cells = false(numel(range_axis_diag), numel(doppler_axis_diag));

        for k = 1 : numel(adsb_aligned)
            idx_truth = find(abs(adsb_aligned(k).t_abs_s - t_block) < 1e-9, 1, 'first');
            if isempty(idx_truth)
                continue
            end

            R_t = adsb_aligned(k).R_excess_m(idx_truth);
            f_t = adsb_aligned(k).f_D_hz(idx_truth);
            if ~isfinite(R_t) || ~isfinite(f_t)
                continue
            end

            [~, r_idx] = min(abs(range_axis_diag - R_t));
            [~, d_idx] = min(abs(doppler_axis_diag - f_t));
            [r_idx, d_idx] = localResolveSyntheticCell(r_idx, d_idx, occupied_cells);

            det_counter = det_counter + 1;
            peak_db = 18 + 10 * mod(det_counter, 2);   % alternating weak/strong targets
            det_range = range_axis_diag(r_idx);
            det_dopp = doppler_axis_diag(d_idx);

            occupied_cells(r_idx, d_idx) = true;
            rdm_white_blk(r_idx, d_idx) = peak_db;
            rdm_after_diag(r_idx, d_idx) = peak_db;

            det_row = [det_range, det_dopp, peak_db, ib, t_block];
            dets_ip = [dets_ip; det_row]; %#ok<AGROW>
            all_track_dets_diag = [all_track_dets_diag; det_row, ip]; %#ok<AGROW>
        end

        blocks_ip(ib).block_num = ib;
        blocks_ip(ib).look_count = 1;
        blocks_ip(ib).t_part_center_s = t_block - part_start_offsets_diag(ip);
        blocks_ip(ib).rdm_whitened_db = rdm_white_blk;
        blocks_ip(ib).row_nf_db = zeros(numel(range_axis_diag), 1);
        blocks_ip(ib).abs_nf_db = 0;
    end
    ...
    part_results_diag(ip).detector_blocks = blocks_ip;
end

assert(~isempty(all_track_dets_diag), ...
    'Synthetic detector replay input should contain at least one detection.');
...
truth_diag_input = buildDetectionTruthDiagnosticInput( ...
    ...
    'Verbose', false);
assert(abs(truth_diag_input.range_cell_m - mean(diff(range_axis_diag))) < 1e-9, ...
    'Truth diagnostic bundle should preserve the detector range-bin spacing.');
assert(abs(truth_diag_input.doppler_bin_hz - mean(diff(doppler_axis_diag))) < 1e-9, ...
    'Truth diagnostic bundle should preserve the detector Doppler-bin spacing.');

truth_diag_input_stale_fc = truth_diag_input;
truth_diag_input_stale_fc.fc = 540e6;
...
saveDetectionTruthDiagnosticInput(truth_diag_input_stale_fc, truth_snapshot_compact, ...
    'IncludeRDMParts', false, ...
    'Verbose', false);
saveDetectionTruthDiagnosticInput(truth_diag_input_stale_fc, truth_snapshot_full, ...
...
assert(abs(diag_output_compact.truth_diag_input.fc - fc) < 1e-9, ...
    'Standalone truth replay should refresh fc from the first radar file header.');
...
assert(abs(diag_output.truth_diag_input.fc - fc) < 1e-9, ...
    'Full truth replay should refresh fc from the first radar file header.');
...
compact_truth_template.fc = 540e6;

detector_replay_input = buildDetectorReplayInput( ...
    config_diag, data_parts_diag, ...
    part_start_offsets_diag, part_end_offsets_diag, part_results_diag, ...
    'TruthDiagnosticInput', compact_truth_template, ...
    'PartDurationS', part_dur_s, ...
    'SessionID', 'synthetic_truth_demo', ...
    'AnalysisLabel', 'Synthetic Detector Replay Unit', ...
    'Verbose', false);

detector_replay_snapshot = fullfile(tmp_dir, 'detector_replay_input.mat');
saveDetectorReplayInput(detector_replay_input, detector_replay_snapshot, ...
    'Verbose', false);
assert(exist(detector_replay_snapshot, 'file') == 2, ...
    'Detector replay snapshot should be saved to disk.');

replay_cases = struct( ...
    'Name', {'baseline', 'tight_snr'}, ...
    'MinSNRDB', {0, 10});

detector_replay_output = runDetectorReplaySweep( ...
    detector_replay_snapshot, ...
    'Cases', replay_cases, ...
    'PlotDetectionTimeSeries', false, ...
    'PlotRDMOverlays', false, ...
    'Verbose', false);

assert(height(detector_replay_output.summary_table) == 2, ...
    'Detector replay sweep should report one row per case.');
assert(detector_replay_output.summary_table.n_detections(1) == size(all_track_dets_diag, 1), ...
    'Baseline detector replay should reproduce the original detection count.');
assert(detector_replay_output.summary_table.n_tp(1) == diag_output.truth_metrics.n_tp, ...
    'Baseline detector replay should preserve the original true-positive count.');
assert(abs(detector_replay_output.case_results(1).truth_diag_output.truth_diag_input.range_cell_m - ...
    mean(diff(range_axis_diag))) < 1e-9, ...
    'Detector replay truth scoring should use detector-derived range-bin spacing.');
assert(abs(detector_replay_output.case_results(1).truth_diag_output.truth_diag_input.fc - fc) < 1e-9, ...
    'Detector replay truth scoring should refresh fc from the first radar file header.');
assert(detector_replay_output.summary_table.n_detections(2) < ...
    detector_replay_output.summary_table.n_detections(1), ...
    'Tighter replay case should reduce the detection count.');

orig_det_key = sortrows(all_track_dets_diag(:, [1, 2, 5, 6]));
replay_det_key = sortrows(detector_replay_output.case_results(1).all_track_dets(:, [1, 2, 5, 6]));
assert(isequal(size(orig_det_key), size(replay_det_key)) && ...
    max(abs(orig_det_key - replay_det_key), [], 'all') < 1e-9, ...
    'Baseline detector replay should reproduce the original range/Doppler/time detections.');
...
function localWriteSyntheticBasebandFile(file_path, fs, fc)
bbw = comm.BasebandFileWriter(file_path, ...
    'SampleRate', fs, ...
    'CenterFrequency', fc, ...
    'Metadata', struct( ...
        'Label', 'SyntheticTruthPipelineTest', ...
        'RecordingUTC', posixtime(datetime(2026, 7, 5, 14, 25, 0, 'TimeZone', 'UTC'))));
bbw(complex(zeros(32, 1, 'single')));
release(bbw);
end

function [r_idx, d_idx] = localResolveSyntheticCell(r_idx, d_idx, occupied_cells)
if ~occupied_cells(r_idx, d_idx)
    return
end

[n_range, n_dopp] = size(occupied_cells);
dopp_offsets = [2, -2, 4, -4, 6, -6];
for k = 1 : numel(dopp_offsets)
    d_try = d_idx + dopp_offsets(k);
    if d_try >= 1 && d_try <= n_dopp && ~occupied_cells(r_idx, d_try)
        d_idx = d_try;
        return
    end
end

range_offsets = [1, -1, 2, -2, 3, -3];
for k = 1 : numel(range_offsets)
    r_try = r_idx + range_offsets(k);
    if r_try >= 1 && r_try <= n_range && ~occupied_cells(r_try, d_idx)
        r_idx = r_try;
        return
    end
end

error('test_adsbTruthPipeline:syntheticCellCollision', ...
    'Could not place a unique synthetic detection cell near [%d, %d].', r_idx, d_idx);
end
