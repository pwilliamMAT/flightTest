...
| **Reusable ADS-B-To-Range-Doppler Projection** | Several workflows need the same two-step truth conversion: project ADS-B tracks into bistatic measurement space, then align that truth to radar query times. `helperProjectADSBTruthToRangeDoppler` is the shared composition layer for that contract. It keeps the authoritative range/Doppler math in `adsbToBistatic` and `alignTruthToRadar`, and adds a flat one-row-per-target-per-query table for validation and spot checks. | `helperProjectADSBTruthToRangeDoppler.m`, `adsbToBistatic.m`, `alignTruthToRadar.m`, `bistaticTruthConventionTest.m` |
