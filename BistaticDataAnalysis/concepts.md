...
| :--- | :--- | :--- |
| **Reference Spectrum and Pilot-Evidence Precheck** | A fast RF-trust diagnostic that reads only a short slice from one radar file, checks reference level, PSD-based pilot evidence, lag dominance, and post-ECA clutter suppression, and then states whether the capture is sufficient to continue passive-radar analysis. The pass/fail pilot gate now comes from a narrow spectral line near the expected ATSC pilot, not from CPI-to-CPI FFT-bin coherence. That is the right bias for passive HDTV because the pilot is the one narrow, always-transmitted spectral marker tied directly to the broadcast waveform that the reference channel is supposed to capture. | `runDirectPathPrecheck.m`, `checkRefQuality.m`, `helperMeasureATSCPilotEvidence.m`, `helperMeasurePilotCoherenceProfile.m`, `helperMeasureCrossCorrelationPeak.m`, `helperMeasureZeroDopplerSuppression.m` |
| **ATSC Pilot Evidence Audit and Stability Advisory** | A follow-on audit that uses one centered Welch PSD for both pilot scoring and plotting, anchors the expected pilot to the ATSC raster and capture tune frequency, keeps mirrored candidates diagnostic-only under the normal-side policy, and then measures residual pilot stability after frequency centering. In this passive bistatic HDTV chain that matters because the reference channel anchors direct-path lag calibration and ECA-C clutter cancellation, so proving that the real transmitted pilot is present at the expected location is more relevant than rewarding whichever FFT bin stayed most phase-stable across CPIs. The returned diagnostic table keeps the selected peak, mirrored evidence, and legacy FFT-bin coherence aligned with the figure annotations. | `helperMeasureATSCPilotEvidence.m`, `runDirectPathPrecheck.m`, `runATSCPilotAuditValidation.m`, `tests/ATSCPilotAuditTest.m` |
...
| **Visualization Profiles for Graphics Stability** | The full session can create many classic figures plus optional `uifigure`/`geoglobe` renderers, which is fragile on machines with limited graphics headroom. The analysis now normalizes those figure paths into `full`, `core`, and `headless` profiles, returns a figure inventory and ranked crash hypotheses, and routes the per-part RDM diagnostics through one slider-based viewer instead of opening one window per part. | `helperResolveVisualizationProfile.m`, `runBistaticAnalysisSession.m`, `analyzeBistaticData.m`, `render_rdm_part.m` |
...
| **RF Sufficiency Audit** | Once timing and simple measurement-space offsets are ruled out, the next question is whether the capture itself is strong and stable enough to support the task. The session audit runs the direct-path precheck across multiple parts, measures reference-channel level, PSD-based ATSC pilot evidence, spectral flatness, lag-domain direct-path dominance, and both the suppression depth and residual post-ECA zero-Doppler ridge. It intentionally de-emphasizes raw inter-channel power asymmetry because surveillance and reference hardware can have very different gain. Weak pilot evidence despite acceptable ADC level is treated as a reference-path sufficiency issue, and a reference-side LNA/preamplifier is considered a valid hardware adjustment. | `checkRefQuality.m`, `helperMeasureCrossCorrelationPeak.m`, `helperMeasureZeroDopplerSuppression.m`, `runDirectPathPrecheck.m`, `summarizeRFQualityAudit.m`, `runSessionRFQualityAudit.m`, `test_rfQualityAudit.m` |
...
| **Toolbox TDOA Measurement Refinement** | The custom CAF detector already supplies a coarse range-delay cell and a Doppler hypothesis. The toolbox timing path keeps that block/time/Doppler context, suppresses the direct path once per block, Doppler-compensates the surveillance block, and then uses `phased.TDOAEstimator` to re-measure only the delay. This isolates the timing/range comparison from detector and localization changes. | `helperEstimateToolboxTDOARange.m`, `helperApplyToolboxTDOARefinement.m`, `runOfflineToolboxBenchmark.m`, `tests/OfflineToolboxBenchmarkTest.m` |
