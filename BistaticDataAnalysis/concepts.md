...
| **Snapshot-Based Fast Iteration** | Once one full session has produced `truth_diag_input`, the truth diagnostics can be rerun directly from the in-memory struct or a saved MAT snapshot. This shortens the edit-test loop from minutes to seconds because only ADS-B loading, bistatic projection, alignment, matching, and plotting are repeated; the expensive IQ, ECA-C, CAF, and CFAR stages are skipped. | `runDetectionTruthDiagnostics.m`, `saveDetectionTruthDiagnosticInput.m`, `test_adsbTruthPipeline.m` |
