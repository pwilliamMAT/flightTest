...
For a hand-rotated environmental RF survey, use [runPlutoAzimuthEnvironmentalScan.m](runPlutoAzimuthEnvironmentalScan.m) or launch it from Linux with [run_pluto_azimuth_environment_scan.sh](run_pluto_azimuth_environment_scan.sh); the field-computer explainer is [Azimuth-Calibration.md](Azimuth-Calibration.md). The scan prompts the operator through 4, 8, or 16 clockwise azimuth bearings, records both N320 channels at each bearing, injects a short 0.2 s Pluto 12-tone no-DC comb during each capture, removes that pulse window from the ambient PSD estimate, and writes `azimuth_summary.csv`, per-tone `calibration_tone_summary.csv`, PNG plots, `summary.txt`, `scan_result.mat`, and a static `index.html` report that can be opened locally in a browser on the field computer. Raw N320 baseband captures are written under `bb_captures_exclude_from_rsync/` with a `__BB_CAPTURE_DO_NOT_RSYNC` filename tag, and each report folder includes `rsync_exclude_large_captures.txt`.
