...
For a hand-rotated environmental RF survey, use [runPlutoAzimuthEnvironmentalScan.m](runPlutoAzimuthEnvironmentalScan.m) or launch it from Linux with [run_pluto_azimuth_environment_scan.sh](run_pluto_azimuth_environment_scan.sh). The scan prompts the operator through 4, 8, or 16 clockwise azimuth bearings, records both N320 channels at each bearing, injects a short 0.2 s Pluto 11-tone comb during each capture, removes that pulse window from the ambient PSD estimate, and writes `azimuth_summary.csv`, PNG plots, `summary.txt`, `scan_result.mat`, and a static `index.html` report that can be opened locally in a browser on the field computer.
