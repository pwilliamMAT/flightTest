...
For the first Phase 2B multitone smoke experiment, use [runPlutoMultitoneStage6Smoke.m](runPlutoMultitoneStage6Smoke.m). It keeps the existing Pluto TX, N320 capture, and `.bb` readback path, but transmits an 8-tone comb and scores every tone on both channels before aggregating the evidence.
