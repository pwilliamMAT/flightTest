...
For the first Phase 2B multitone smoke experiment, use [runPlutoMultitoneStage6Smoke.m](runPlutoMultitoneStage6Smoke.m). It keeps the existing Pluto TX, N320 capture, and `.bb` readback path, but transmits an 8-tone comb and scores every tone on both channels before aggregating the evidence. After the capture syncs, use [reviewPlutoMultitoneCapture.m](reviewPlutoMultitoneCapture.m) to regenerate the score and write durable `result.mat`, `summary.txt`, and `summary.png` review artifacts without rerunning hardware.
