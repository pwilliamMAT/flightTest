...
%
%  Example:
%    info = runLocalHDTVCapture('Gain', [16 16], ...
...
addParameter(p, 'Gain', [16 16], @(x) isnumeric(x) && (isscalar(x) || numel(x) == 2));
addParameter(p, 'AntennaPorts', ["RF0:RX2", "RF1:RX2"], @localIsTwoTextValues);
addParameter(p, 'ChannelRoles', ["surveillance", "reference"], @localIsChannelRolePair);
addParameter(p, 'OperatorSetupNotes', "", @(x) ischar(x) || isstring(x));
...
addParameter(p, 'CaptureFunction', @log_iq_n320_2antennas, ...
    @(x) isa(x, 'function_handle'));
parse(p, varargin{:});
opts = p.Results;

operator_setup_notes = helperResolveOperatorSetupNotes(opts.OperatorSetupNotes);
channel_mapping = helperBuildChannelMapping( ...
    opts.AntennaPorts, opts.ChannelRoles, opts.Gain);
...
capture_args = { ...
    ...
    'lo', opts.LOOffset_Hz, ...
    'gain', opts.Gain, ...
    'antennas', string(opts.AntennaPorts), ...
    'roles', lower(string(opts.ChannelRoles)), ...
    'operator_setup_notes', operator_setup_notes, ...
...
[session_id, written_files] = opts.CaptureFunction(capture_args{:});
...
capture_info = struct( ...
    ...
    'gain', opts.Gain, ...
    'antenna_ports', reshape(string(opts.AntennaPorts), 1, []), ...
    'channel_roles', lower(reshape(string(opts.ChannelRoles), 1, [])), ...
    'operator_setup_notes', operator_setup_notes, ...
...
function tf = localIsTwoTextValues(value)
if ~(ischar(value) || isstring(value) || iscellstr(value))
    tf = false;
    return
end
values = string(value);
tf = numel(values) == 2 && all(strlength(values) > 0);
end

function tf = localIsChannelRolePair(value)
if ~(ischar(value) || isstring(value) || iscellstr(value))
    tf = false;
    return
end
roles = lower(reshape(string(value), 1, []));
tf = numel(roles) == 2 && ...
    isequal(sort(roles), sort(["reference", "surveillance"]));
end
