...
addParameter(p, 'PlutoTxGain_dB', NaN, @(x) isnumeric(x) && isscalar(x) && (isnan(x) || (x >= -89.75 && x <= 0)));
...
for stepIndex = 1:opts.NumAzimuthSteps
    ...
    if opts.DryRun
        ...
        captureInfo = localCaptureN320WithPlutoPulse( ...
            ...
            'PlutoTxGain_dB', opts.PlutoTxGain_dB, ...
...
opts.PlutoTxGain_dB = localSetting(settings, 'pluto_tx_gain_db', opts.PlutoTxGain_dB);
...
settings = struct( ...
    ...
    'pluto_tx_gain_db', double(opts.PlutoTxGain_dB), ...
...
addParameter(p, 'PlutoTxGain_dB', NaN, @(x) isnumeric(x) && isscalar(x));
