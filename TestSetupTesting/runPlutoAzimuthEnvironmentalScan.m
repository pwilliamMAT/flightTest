...
try
    ...
    localCaptureAndWriteSegments(bbrx, writer, opts.PulseStartDelay_s, opts.CaptureSegment_s);
    ...
    txContext = helperPlutoToneStartTx( ...
        ...
        'Verbose', opts.Verbose);
    localCaptureAndWriteSegments(bbrx, writer, opts.PulseDuration_s, opts.CaptureSegment_s);
    localReleaseTransmitter(txContext);
    txContext = struct();

    remainingDuration = opts.CaptureDuration_s - opts.PulseStartDelay_s - opts.PulseDuration_s;
    localCaptureAndWriteSegments(bbrx, writer, remainingDuration, opts.CaptureSegment_s);
...
function localCaptureAndWriteSegments(receiver, writer, durationSeconds, maxSegmentSeconds)
remainingSeconds = double(durationSeconds);
while remainingSeconds > 1e-9
    segmentSeconds = min(remainingSeconds, double(maxSegmentSeconds));
    data = capture(receiver, seconds(segmentSeconds));
    writer(data);
    remainingSeconds = remainingSeconds - segmentSeconds;
end
end
