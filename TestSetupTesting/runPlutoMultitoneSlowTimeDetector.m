...
slowTime = struct( ...
    ...
    'tonewise_peak_detection', localBuildTonewisePeakDetection( ...
        integration.slow_time_frequency_hz(:), integration.tone_offsets_hz(:), refNormPower, survNormPower), ...
    'tone_support_at_peak', toneSupport);
end

function tonewisePeak = localBuildTonewisePeakDetection(frequencyHz, toneOffsetsHz, refNormPower, survNormPower)
jointPower = sqrt(refNormPower .* survNormPower);
[peakPower, peakIdx] = max(jointPower, [], 2);
peakFrequencyHz = frequencyHz(peakIdx);
floorPower = median(jointPower, 2, 'omitnan');
peakContrastDb = 10 * log10(peakPower ./ max(floorPower, eps) + eps);
...
toneTable = table( ...
    ...
    peakContrastDb(:), ...
    10 * log10(refNormPower(sub2ind(size(refNormPower), (1:size(refNormPower, 1)).', peakIdx(:))) + eps), ...
    10 * log10(survNormPower(sub2ind(size(survNormPower), (1:size(survNormPower, 1)).', peakIdx(:))) + eps), ...
    'VariableNames', { ...
        'Tone_kHz', ...
        'PeakSlowTimeFrequency_Hz', ...
        'JointPeakPower_dB', ...
        'JointPeakContrast_dB', ...
        'REF_PowerAtTonePeak_dB', ...
        'SURV_PowerAtTonePeak_dB'});

combPeakPower = mean(peakPower, 'omitnan');
combFloorPower = mean(floorPower, 'omitnan');
tonewisePeak = struct( ...
    'comb_peak_power_db', 10 * log10(combPeakPower + eps), ...
    'comb_floor_power_db', 10 * log10(combFloorPower + eps), ...
    'comb_peak_contrast_db', 10 * log10(combPeakPower / max(combFloorPower, eps) + eps), ...
    'median_peak_frequency_hz', median(peakFrequencyHz, 'omitnan'), ...
    'peak_frequency_std_hz', std(peakFrequencyHz, 'omitnan'), ...
    'median_tone_peak_contrast_db', median(peakContrastDb, 'omitnan'), ...
    'tone_table', toneTable);
end
...
fprintf(fid, '\nTonewise residual-compensated peaks:\n');
localWriteNumericTable(fid, detection.slow_time_detection.tonewise_peak_detection.tone_table);
...
lines = [
    ...
    "Tonewise compensated peak contrast " + ...
        compose("%.1f", slowTime.tonewise_peak_detection.comb_peak_contrast_db) + " dB"
    "Tonewise peak frequency std " + ...
        compose("%.1f", slowTime.tonewise_peak_detection.peak_frequency_std_hz) + " Hz"
    ];
end
...
disp(detection.slow_time_detection.tonewise_peak_detection.tone_table);
...
nexttile(tl, 2);
bar(toneOffsetsKhz, ...
    [slowTime.tonewise_peak_detection.tone_table.JointPeakContrast_dB, ...
    slowTime.tone_support_at_peak.Joint_NormalizedPower_dB]);
grid on;
xlabel('Expected tone offset (kHz)');
ylabel('Joint tone evidence (dB)');
title('Common-bin versus tonewise peak');
legend({'Tonewise peak contrast', 'Common-bin joint power'}, 'Location', 'best');
