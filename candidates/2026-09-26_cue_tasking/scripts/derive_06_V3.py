...
<tr><td><span class="evidence-id">STAT-013</span></td><td>Truth separation for cued collections</td><td><span class="tag progress">In Progress</span></td><td>Owner confirmed on 2026-09-26 that cues now choose when and with which tower to collect, and that the Tracker may use cues to help association. Rule proposed as CR-9: label every capture (<code>cued</code>, <code>uncued</code>, …) and every product (<code>truth_blind</code>, <code>cue_aided</code>); only truth-blind products count as independent evidence. See <a href="{TRUTH}">the rule</a>.</td><td>Until the rule text is accepted, no result from a cued collection may be presented as independent detection evidence.</td></tr>
</tbody>
</table>
</div>
...
EDITS: list[tuple[str, str]] = [
    ...
    (
        ...
        '<tr><td><span class="evidence-id">STAT-013</span></td><td>Truth separation for cued collections</td><td><span class="status-chip status-progress">In Progress</span></td><td>Rule proposed as CR-9 (labels for collections and products).</td><td>Until accepted, cued results are not independent detection evidence.</td></tr>',
    ),
    # Capability matrix.
    (
    ...
    (
        ...
        "<li>Truth remains post hoc; it cannot steer Passive Map formation, thresholding, or nonmaximum suppression.</li>"
        f"<li>ADS-B cues may choose when and on which illuminator to collect, and the Tracker may use cues to help association. The proposed rule (CR-9, <a href=\"{TRUTH}\">truth separation</a>) labels every capture and product and counts only truth-blind products as independent evidence; until it is accepted, no cued result is independent detection evidence. <span class=\"evidence-id\">STAT-013</span></li>"
    ...
    (
        ...
        '<tr><td><span class="evidence-id">STAT-010</span> <span class="evidence-id">STAT-012</span> <span class="evidence-id">STAT-013</span> Cueing and timing infrastructure</td><td>Decided 2026-09-26: keep internet NTP with a 0.1 s tolerance; reseat the GPS/PPS component (hardware to-do); accept or amend the CR-9 truth-separation rule; Pat to review the CueListener branch; then build Resource Manager scheduling and <code>collection_task</code> with <code>cue_ref</code>. The re-check of earlier ADS-B timing stays open.</td><td>Cue interface Verified live (STAT-009); Pi clock was 14.9 s slow before NTP; <code>collection_task</code> is Proposed.</td><td>An accepted CR-9, a repeatable clock check, and a first collection scheduled from a cue and linked to it.</td><td>Enabling work; it does not relieve the map-contract or R5B collection-suitability blockers.</td></tr>\n'
        '<tr><td><span class="evidence-id">STAT-007</span> Future Tracking / Localization</td>',
    ),
